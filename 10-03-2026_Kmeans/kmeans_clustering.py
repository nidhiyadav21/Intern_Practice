...
class CustomerSegmentation:
    ...
    def plot_outliers(self):
        ...
        try:
            ...
            for col_name in numeric_col_list:
                ...
                clean_series = self.df[col_name].dropna()  # Creates a copy of the column and removes any NaN (null) values.

                # Check if the series has any data points to plot
                if len(clean_series) > 0:            #A safety check to ensure there is actually data left to plot after dropping nulls.
    ...
    def preprocess_data(self):
            ...
            try:
                ...
                for col in ['Work_Experience', 'Family_Size']:
                    ...
                    self.df[col] = np.where(self.df[col] > upper_limit, upper_limit, self.df[col])   # This is capping - If a value is higher than the 99th percentile, it is reset to that limit

                self.df.drop(columns=['ID', 'Profession', 'Var_1'], inplace=True, errors='ignore')

                # 1. Map Ordinal Features (CRITICAL for K-Means)
                res_map = {'Low': 1, 'Average': 2, 'High': 3}         #Defines a dictionary to convert text-based ratings into meaningful numbers.
                if 'Spending_Score' in self.df.columns:
                    self.df['Spending_Score'] = self.df['Spending_Score'].map(res_map)  #This is Ordinal Encoding
    ...
    def build_pipeline(self, k=4):
        ...
        try:
            ...
            kmeans = KMeans(n_clusters=k, random_state=self.random_state, n_init=10) #runs the algorithm 10 times with different starting points and picks the best one
            self.pipeline = make_pipeline(transformer,PCA(n_components=0.95), kmeans) #Reduces data complexity by keeping only the most important features that capture 95% of the information.
    ...
    def find_optimal_k(self, max_k=10):
        ...
        try:
            ...
            plt.plot(K, inertia, 'bx-')  #inertia-Sum of Squared Distances
    ...
    def train_data(self):
        ...
        try:
            print("Training data...")
            self.pipeline.fit(self.df)  #It sends the raw data through the entire pipeline created earlier (Mapping->Scaling->PCA-> K-Means). The model "learns" the patterns and finds the best center point for each group.
    ...
    def evaluate_model(self):
        ...
        try:
            ...
            X_transformed = self.pipeline.named_steps['columntransformer'].transform(self.df) #This takes the original data and processes it (scaling/encoding) exactly how the model expects it
            labels = self.pipeline.named_steps['kmeans'].labels_

            score = silhouette_score(X_transformed, labels)
            print(f"--- Optimized Silhouette Score: {score:.4f} ---")
            self.df['Cluster'] = labels    #Create a new column in the data to show each customer's group
            print(self.df.head())
